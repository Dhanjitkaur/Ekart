...
def makepayment(request):
    userid=request.user.id
    client = razorpay.Client(auth=("rzp_test_66dnClbpyoOqq6", "QJQNJZjGeUu1BLj4jaMkyOSA"))
...
def storedetails(request):
    ...
    #print(sign)
    email=u[0].email
    msg="order placed successfull.Details are payment ID:"+pay_id+" and order Id is:"+order_id,
    send_mail(
        "order status-Ekart",
        "Your Order id placed successfully",
        settings.EMAIL_HOST_USER,
        ["dhanjitkaur08@gmail.com"],
        fail_silently=False,
    )
